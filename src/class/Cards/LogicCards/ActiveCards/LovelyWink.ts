...
export class LovelyWink extends LogicCard {
  ...
  public applyLogicEffect = (props: LogicProps): LogicProps => {
    ...
    if (this.grade === 0) {
      result = this.applyLogicEffectChangeByPercent(props, 60);
    } else if (this.grade === 1) {
      result = this.applyLogicEffectChangeByPercent(props, 80);
    } else {
      result = props;
    }
    return result;
  ...
  public copyCard(): LogicCard {
    return new LovelyWink(this.grade);
  }
}
